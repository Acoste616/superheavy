...
const CustomerDecoderEngine = require('./backend/backend/CustomerDecoderEngine.js');
const CustomerJourneyTracker = require('./backend/backend/CustomerJourneyTracker.js');
const ProbabilityAnalyzer = require('./backend/backend/ProbabilityAnalyzer.js');
...
app.get('/api/triggers', async (req, res) => {
    try {
        ...
        const proxyReq = http.request(options, (proxyRes) => {
            ...
            proxyRes.on('end', () => {
                try {
                    ...
                    res.status(500).json({
                        success: false,
                        error: 'Invalid response from trigger service'
                    });
                }
            });
        });
        ...
        proxyReq.on('error', (error) => {
            ...
            res.status(503).json({
                success: false,
                error: 'Trigger service unavailable',
        ...
        console.error('Trigger proxy setup error:', error);
        res.status(500).json({
            success: false,
